...
/**
 * The RemoteErrorEvent is an event that can be fired by remote clients via RPC calls with an error level
 * @author dbattams
 * @version $Id$
 */
final class RemoteErrorEvent implements SageEvent {
	/**
	 * The metadata for this event.
	 */
	static final SageEventMetaData EVENT_METADATA = new SageEventMetaData(RemoteErrorEvent.class.getCanonicalName(), "Remote Error Alerts", "An error alert triggered by a remote application via the SageAlert XML-RPC server.");
	
	private String title;
	private String desc;
	private String source;
	
	RemoteErrorEvent(String title, String desc, String source) {
		this.title = title;
		this.desc = desc;
		this.source = source;
	}
